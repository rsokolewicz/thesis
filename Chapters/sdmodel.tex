...
\begin{figure}
    ...
    \caption{Switching behavior for a rashba ferromagnet, subject to alternating current pulses. Gray areas correspond to a constant current flowing in the direction of $\pm\hat{x}$, where the sign alternates between each succesive box. In the white area the current is switched off. The graph corresponds to numerically solving Eq.~(\ref{eq:sd:ferro}) with values $c_1=0.2$, $j=|\bb{j}|=1$ and $\alpha=0.1$. In the top panel we choose a value of $c_2=0.25$ corresponding to a current overcoming the Gilbert damping making switching possible. In the bottom panel we choose a value of $c_2=0.05$ which corresponds to the opposite case: an insufficiently strong current and therefore no switching. A magnetocrystalline anisotropy term $-K \bb{n}\times\bb{n_y}$ with $K=1$ was included in Eq.~(\ref{eq:sd:ferro}).} 
