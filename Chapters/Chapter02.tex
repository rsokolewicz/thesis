...
\be
\label{sdensity}
\bb{s}(\bb{r})= \frac{1}{2}\int \frac{dE}{2\pi \hbar} \s_\text{A} f_A(E)  \sum_{\alpha}  \Psi^{A\;\dagger}_{\alpha,E} \bb{\sigma}_{\sigma\sigma'} \Psi^{A}_{\alpha,E}, 
 \e
...
\be
v= \frac{3}{2} wa/\hbar, \qquad \alpha_\text{R}= \frac{3}{2} \lambda , \qquad \Delta=JS.
\e
...
\be
\label{TA}
\bb{T}^\text{A}= \frac{J\mathcal{A}}{\hbar} \bb{n}^\text{A}\times\delta\bb{s}^\text{A}= \frac{J\mathcal{A}}{\hbar} \lt(\bb{\ell}+\bb{m}\rt)\times\lt(\delta\bb{s}_++\delta\bb{s}_-\rt),
\e
...
\label{AFMEOM2}
\begin{align}
&\dot{\bb{\ell}} = -(J_\text{ex}/2\hbar)\, \bb{\ell}\times\bb{m} +\bb{H}\times\bb{\ell}+\bb{T}^\text{A},\\
...
\caption{Time evolution of the in-plane N\'eel vector $\bb{n}_\parallel(t)$ in the symmetric model (top panel) and asymmetric model (bottom panel) induced by a sequence of current pulses alternating between $-\hat{\bb{x}}$ and $\hat{\bb{y}}$ directions (gray areas). The results are obtained from Eqs.~(\ref{EOMAFM10}) for  the parameters $\alpha=0.005$, $\hbar\eta j/J_\text{ex}=0.05$, $E/w=0.3$ and $n_\text{imp}=0.3$.}
\label{fig:switching}
\end{figure}
