...
\be
\bb{\ell}=\lt(\bb{m}^\textrm{A}-\bb{m}^\textrm{B}\rt)/2,\qquad \bb{m}= \lt(\bb{m}^\textrm{A}+\bb{m}^\textrm{B}\rt)/2,
\e
...
\be
\label{chap02:low}
H^\textrm{eff}_0= v\, \bb{p}\cdot\bb{\Sigma}+\alpha_\textrm{R}\lt[\bb{\sigma}\times\bb{\Sigma}\rt]_{\hat{z}} - \Delta_\text{sd}\,\bb{\ell}\cdot\bb{\sigma}\,\Sigma_z\Lambda_z,
\e
...
\be
v= \frac{3}{2} wa/\hslash, \qquad \alpha_\textrm{R}= \frac{3}{2} \lambda , \qquad \Delta_\text{sd}=JS.
\e
...
\be
\label{chap02:s_SOT}
\delta\bb{s}_+= \frac{\Delta_\text{sd}\lambda}{2\pi evw^2} a_I\,\hat{\bb{z}}\times \bb{j}, 
\e
...
\e
where $\eta=J\mathcal{A}\Delta_\text{sd}\lambda /evh w^2$. 
...
\delta\bb{s}_+= \frac{\Delta_\text{sd}\lambda}{2\pi evw^2}
...
\end{align}
\eml
where $\eta=J\mathcal{A}\Delta_\text{sd}\lambda /evh w^2$ is the same as in Eq.~(\ref{chap02:metal_SOT}). 
...
\be
\label{chap02:TA}
\bb{T}^\textrm{A}= \frac{J\mathcal{A}}{\hslash} \bb{m}^\textrm{A}\times\delta\bb{s}^\textrm{A}= \frac{J\mathcal{A}}{\hslash} \lt(\bb{\ell}+\bb{m}\rt)\times\lt(\delta\bb{s}_++\delta\bb{s}_-\rt),
\e
...
&+\kappa\, \delta\bb{s}^\textrm{A}[\bb{m}^\textrm{A}_\perp,-\bb{m}^\textrm{A}_\parallel]+\zeta\kappa\, \delta\bb{s}^\textrm{A}[-\bb{m}^\textrm{A}_\perp,-\bb{m}^\textrm{A}_\parallel]\Big), 
\end{align}
...
% In order to model AFM magnetization dynamics we adopt the equations of motion (\ref{chap02:AFMEOM}) for the unit vectors $\bb{m}^\textrm{A}$ and $\bb{m}^\textrm{B}$  with Gilbert damping terms included,
% \beml
% \label{chap02:EOMAFM10}
% \begin{align}
% \dot{\bb{m}}^\textrm{A} &= \frac{J_\textrm{ex}}{2\hslash}\bb{m}^\textrm{B}\times \bb{m}^\textrm{A}+ \frac{J\mathcal{A}}{\hslash}\, \bb{m}^\textrm{A}\times \bb{s}^\textrm{A} +\alpha\, \bb{m}^\textrm{A}\times \dot{\bb{m}}^\textrm{A},\\
% \dot{\bb{m}}^\textrm{B}&= \frac{J_\textrm{ex}}{2\hslash} \bb{m}^\textrm{A}\times \bb{m}^\textrm{B}+ \frac{J\mathcal{A}}{\hslash}\, \bb{m}^\textrm{B}\times \bb{s}^\textrm{B}+\alpha\, \bb{m}^\textrm{B}\times \dot{\bb{m}}^\textrm{B},
% \end{align}
% \eml
% where we take $\alpha=0.005$ for the strength of the Gilbert damping.  

% In Fig.~\ref{chap02:fig:switching} we illustrate the AFM dynamics induced by short current pulses of the density $j=0.05\,J_\textrm{ex}/\eta\hslash$, which are applied in alternating directions $-\hat{\bb{x}}$ and $\hat{\bb{y}}$. The parameters of spin-orbit torques are derived for the Fermi energy $E =0.3\,t_h$ (in the metal regime) and for the impurity concentration $n_\textrm{imp}=0.3$ which corresponds to placing impurities on $30\%$ of lattice sites.   
