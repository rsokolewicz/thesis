...
\begin{figure}
\centering
\centerline{\includegraphics[width=0.7\columnwidth]{{gfx/system_m_n.pdf}}}
\caption{A model of Rashba honeycomb antiferromagnet with two sublattices, $A$ and $B$, and on-site exchange interaction between localized moments and conduction electrons (see Eq.~\ref{chap03:ex}). We refer to a specific coordinate system where $\hat{\bb{x}}$-axis is  along the in-plane component of $\bb{n}$.}
...
The presence of the undamped mode identified here, illustrates how lowering the symmetry of the electronic bath (by spin-orbit interaction) may induce a conservation law in the localized spin subsystem. Based on this microscopic mechanism we provide qualitative arguments in favor of a generality of the giant Gilbert damping anisotropy in a 2D metallic AFM with spin-orbit coupling. Even though the undamped mode cannot be associated with a single spin-wave or a magnon, its presence has a strong impact on the nonequilibrium N\'eel vector dynamics in 2D Rashba AFMs.
...
\begin{align}
\label{chap03:ndot}
\dot{\bb{n}}  & = -2J_\textrm{ex}S\, \bb{n}\times\bb{m} +\bb{H}\times\bb{n}+\bb{n}\times\bb{s}^++\bb{m}\times\bb{s}^-,\\
\label{chap03:mdot}
\dot{\bb{m}}  & = \bb{H}\times\bb{m}+\bb{m}\times\bb{s}^++\bb{n}\times\bb{s}^-,
\end{align}
\eml
where $\bb{s}^{\pm}= J_\text{sd} \mathcal{A}\,(\bb{s}^\textrm{A}\pm\bb{s}^\textrm{B})/2\hslash$. The effects arising from the anisotropy $K$ are dropped for the time being. 
...
\begin{figure}
\centering
\centerline{\includegraphics[width=0.7\columnwidth]{{articles/misha_paper/fig2.pdf}}}
...
\begin{align}
\label{chap03:ndot2}
 & \dot{\bb{n}} = -2J_\text{ex}S\, \bb{n}\!\times\!\bb{m} +\bb{H}\!\times\!\bb{n}+\bar{\alpha}_m\,\bb{n}\!\times\!\dot{\bb{m}}+\alpha_n\,\bb{m}\!\times\! \dot{\bb{n}},\\
...
\e
Here $\bb{\Sigma}$, $\bb{\Lambda}$, and $\bb{\sigma}$ are the vectors of Pauli matrices in sublattice, valley and spin space, respectively, $v$ is the characteristic Fermi velocity, while $\lambda$ and $\Delta_{\text{sd}}=J_\text{sd}S$ are the energy scales characterizing the strength of Rashba spin-orbit coupling and $s$-$d$-like exchange energy, correspondingly. 
...
\end{align}
\eml
where the principal axes correspond to choosing $\hat{\bm{x}}$ direction along $\bb{n}_\para$ (see Fig.~\ref{chap03:fig:lattice}). In the deep metal regime, and for a general direction of $\bb{n}$, this anisotropy is evidently small
\be
\label{chap03:ani}
...
\begin{multline}
\label{chap03:sp}
\delta\bb{s}^+= a(n_z^2)\;\hat{\bb{z}}\times \bb{j} + b(n_z^2)\, \bb{n}_\para\times(\bb{n}_\para\times \lt(\hat{\bb{z}}\times \bb{j}\rt))\\
+ c(n_z^2)\, \bb{n}_\para \times(\bb{n}_\perp\times \lt(\hat{\bb{z}}\times \bb{j}\rt)),
\end{multline}
...
where
\be
a_0=\frac{J_\text{sd}\mathcal{A}}{e\hslash v}\,\frac{\lambda}{\ep_F}, \qquad \bar{\lambda}=\frac{\lambda}{\ep_F},\qquad\delta=\frac{\Delta_{\text{sd}}}{\ep_F}.
\e
In the metal regime, $\ep_F\gg \lambda+\Delta_{\text{sd}}$, the results of Eqs.~(\ref{chap03:SOT}) are reduced to 
\be
\label{chap03:SOTlargeE}
a=\frac{J_\text{sd}\mathcal{A}}{e\hslash v}\,\frac{\lambda}{\ep_F}, \qquad b=-c=2\,\frac{J_\text{sd}\mathcal{A}}{e\hslash v}\,\frac{\lambda}{\ep_F}
...
\begin{figure}
\centering
\centerline{\includegraphics[width=0.7\columnwidth]{{articles/misha_paper/fig3.pdf}}}
...
\begin{multline}
\delta\bb{s}^-= \frac{J_\text{sd}\mathcal{A}}{e\hslash v}\,\frac{\lambda}{\ep_F}
\lt(\frac{\Delta_{\text{sd}}}{\ep_F}\rt)^2\,
\Big[2\,\bb{n}_{\perp} \!\times\!(\bb{m}_{\perp} \!\times\! (\hat{\bb{z}}\!\times\!\bb{j}))
\\
+2\,\bb{m}_{\para} \!\times\!(\bb{n}_{\perp} \!\times\! (\hat{\bb{z}}\!\times\!\bb{j})) -3\,\bb{n}_{\para}\!\times\! (\bb{m}_{\perp}\! \times\! (\hat{\bb{z}}\!\times\!\bb{j}))\n \Big],
\label{chap03:stagg}
\end{multline}
...
\begin{align}
 & \delta\bb{s}^+= \alpha_m^\para\,\dot{\bb{m}}_\para + \gamma \bb{\Gamma}_{mm}+ \gamma\bb{\Gamma}_{mn},\\
...
\begin{align}
 & \alpha_m^\para= 2\,\frac{\ep_F\tau}{\hslash}\;\frac{\mathcal{A} J_\text{sd}^2 S}{\pi\hslash^2v^2}\; \lt[1-\frac{\Delta_{\text{sd}}^2}{\ep_F^2}(2+n^2_z) + \dots\rt],\\
 & \alpha_n^\perp = \frac{\ep_F\tau}{\hslash}\;\frac{\mathcal{A} J_\text{sd}^2 S}{\pi\hslash^2v^2} \,\lt[\lt(\frac{\lambda}{\ep_F}\rt)^2+\dots\rt],\\
 & \gamma= 2\,\frac{\ep_F\tau}{\hslash}\;\frac{\mathcal{A} J_\text{sd}^2 S}{\pi\hslash^2v^2}\,\lt[\lt(\frac{\Delta_{\text{sd}}}{\ep_\textrm{F}}\rt)^2+\dots\rt],
\end{align}
\eml
...
\begin{align}
\bb{\Gamma}_{mm}= & \,
\bb{n}\!\times\!(\bb{n}\!\times\!\dot{\bb{m}}) + \bb{n}_\para\!\times\!(\bb{n}_\para\!\times\!\dot{\bb{m}}_\para)  
- 2 \bb{n}_\para\!\times\!(\bb{n}_\para\!\times\!\dot{\bb{m}}_\perp),\\
\bb{\Gamma}_{mn}= & \,
\bb{n}\!\times\!(\bb{m}_\para\!\times\!\dot{\bb{n}}_\perp) 
- \bb{m}_\perp\!\times\!(\bb{n}_\para\!\times\!\dot{\bb{n}}_\perp)
+\bb{n}_\perp\!\times\!(\bb{m}_\perp\!\times\!\dot{\bb{n}}_\para) \n\\
 & - \bb{n}_\para\!\times\!(\bb{m}_\para\!\times\!\dot{\bb{n}}_\para)
- 3\bb{m}_\perp\!\times\!(\bb{n}_\perp\!\times\!\dot{\bb{n}}_\para),\\
\bb{\Gamma}_{nm}= & \,2\bb{n}_\para\!\times\!(\bb{m}_\para\!\times\!\dot{\bb{m}}_\perp)+ 
2\bb{m}_\para\!\times\!(\bb{n}_\perp\!\times\!\dot{\bb{m}}_\para)
-\bb{n}_\perp\!\times\!(\bb{m}_\perp\!\times\!\dot{\bb{m}})\n\\
 & +2 \bb{m}_\perp\!\times\!(\bb{n}\!\times\!\dot{\bb{m}})
+ \bb{m}_\para\!\times\!(\bb{n}\!\times\!\dot{\bb{m}}_\perp) 
- \bb{m}_\perp\!\times\!(\bb{n}_\perp\!\times\!\dot{\bb{m}}_\para),\\
\bb{\Gamma}_{nn}= & \,-\bb{m}\times(\bb{n}\times\dot{\bb{n}}_\para).
\end{align}
\eml
...
\begin{align}
 & \dot{\bb{n}} = -2J_\text{ex}S\, \bb{n}\!\times\!\bb{m} +\bb{H}\!\times\!\bb{n}+\bar{\alpha}^\para_m\,\bb{n}\!\times\!\dot{\bb{m}}_\para+\alpha_n^\perp\,\bb{m}\!\times\! \dot{\bb{n}}_\perp,\n\\
 & \dot{\bb{m}} =\bb{H}\times\bb{m}+\alpha_n^\perp\,\bb{n} \times \dot{\bb{n}}_\perp
+\bar{\alpha}_m^\para \bb{m} \times \dot{\bb{m}}_\para,
...
Indeed, it can be directly seen from Eqs.~(\ref{chap03:AFMEOM3}) that a nonequilibrium state with  $\bb{m}=m \hat{\bb{z}}$ and $\bb{n}=\bb{n}_\para$ becomes undamped in the absence of external field $\bb{H}=0$. Such a state corresponds to the undamped N\'eel vector precession around $\hat{\bb{z}}$ axis with a frequency given by $J_\textrm{ex} m$. The state clearly survives in the presence of easy plane magnetic anisotropy in the AFM. We believe that such a phenomenon remains to be rather generic for a variety of 2D or layered AFM systems with strong spin-orbit coupling of Rashba type. 
...
We also demonstrate that a finite in-plane projection $\bb{n}_\para$ of the N\'eel vector is responsible for a weak anisotropy of conductivity and spin-orbit torques for Fermi energies approaching the band edge, $\ep_F \sim \Delta_{\text{sd}}+\lambda$. This anisotropy is, however, absent in the metallic regime. 
