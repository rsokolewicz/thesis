...
currents in devices without any magnetic contacts. A key concept is the spin torque induced by an
electrical current, which can be used to the reverse the direction of magnetization direction and
to drive magnetic domain walls. We distinguish two types of current induced current-induced spin
torque: spin-transfer torque (STT) [1–4] and spin-orbit torque (SOT) [5–8]. STTs generally vanish in
uniform magnets and are therefore proportional to gradients in magnetization. These torques arise
from the transfer of spin angular momentum from a spin-current spin current to the spin degree of
freedom of a ferromagnetic region. The SOT, on the other hand, is in general proportional to
magnetization and is mediated by spin-orbit interactions.
...
Here we present a simple model that includes disorder and does not depend on the
existence of diffusive electrons. The model consists of localized magnetic
moments and conducting electrons on a honeycomb lattice, that interact with each other
through the s—d interaction. By calculating the spin-polarization of the conducting
electrons in response to an electric field, we can derive coupled Landau-
Lifshitz type equations that describe the dynamics of the (antiferro)magnetic order in the
system under the influence of electric currents. The coefficients found in these equations
are derived using a non-equilibrium quantum field theory framework and are directly
related to material properties such as the chemical potential and s—d energy. We illustrate
the use of our model by studying the driving of magnetic domain walls by means of an
electric current.
%%%
...
\section{s--d model}
An attractive model that qualitatively describes magnetization dynamics, driven by electric currents is the s--d model. The model consists of two systems: a lattice with a magnetic subsystem and conducting electrons. A phenomenological parameter called the $\Delta_\text{sd}$ energy, couples the magnetic subsystem with the spin of the conducting electrons, which allows the transfer of spin angular momentum from one to the other. This simple model is enough to describe a whole range of phenomena, ranging from the Kondo effect to ... 
